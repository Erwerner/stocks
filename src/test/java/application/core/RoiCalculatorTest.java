...
public class RoiCalculatorTest extends UnitTest {
    ...
    public void when_roi_is_calculated_double_each_day_then_roi_is_2_power_to_356() {
        ...
        double act = new RoiCalculator().calcRoiFromTwoPoints(point1, point2);
    ...
    @Test
    public void when_calc_roi_for_not_sold_asset_at_date_then_result_is_correct() {
        ArrayList<WknPoint> wknPoints = new ArrayList<>();
        LocalDate startDate = LocalDate.of(2001, 1, 1);
        wknPoints.add(new WknPoint(startDate.plusDays(3), 2.0));
        wknPoints.add(new WknPoint(startDate.plusDays(2), 1.0));
        wknPoints.add(new WknPoint(startDate.plusDays(1), 1.0));
        wknPoints.add(new WknPoint(startDate, 1.0));
        WknkRow wknkRow = new WknkRow(wknPoints);
        Asset asset = new Asset("wkn", wknkRow);
        asset.addBuy(new AssetBuy("", startDate, 1000, 0.0, 1.0, null, null));
        AssetBuy assetBuy = asset.getAllBuys().get(0);
        Double act = cut.calcRoiForAssetBuyAtDate(asset, assetBuy, 2, startDate.plusDays(3), 0, false);
        Double expected = Math.pow(2, 365);
        assertEquals(expected, act);
    }
    ...
    public void when_calc_roi_for_asset_with_two_buys_then_result_is_correct() {
        ...
        Double act = cut.calcWeightedRoiForAssetsBuyAtDate(assets, 5, startDate.plusDays(4), 0, false);
        Double expected = Math.pow(2, 365);
        assertEquals(expected, act);
    }
    ...
    public void when_calc_roi_for_alot_buys_then_result_is_correct() {
        ...
        assets.add(asset2);
        assets.add(asset3);
        Double act = cut.calcWeightedRoiForAssetsBuyAtDate(assets, 366, endDate, 0, false);
        Double exp = 0.3;
        assertEquals(exp, act);
    }
    ...
    public void when_calc_roi_for_alot_buys_then_buys_in_past_are_ignored() {
        ...
        Asset asset2 = makeAsset(1000, 1.3, startDate, endDate);
        Asset asset3 = makeAsset(2000, 1.4, startDate, endDate);
        asset3.addBuy(new AssetBuy("wkn", startDate, 2000, 0.0, 1.4, startDate.plusDays(1), 1.4));
        List<Asset> assets = new ArrayList<>();
        assets.add(asset1);
        assets.add(asset2);
        assets.add(asset3);
    ...
    private Asset makeAsset(int amount, double endValue, LocalDate startDate, LocalDate endDate) {
        ...
        wknPoints.add(new WknPoint(startDate, 1.0));
        WknkRow wknkRow = new WknkRow(wknPoints);
        asset1 = new Asset("wkn", wknkRow);
        asset1.addBuy(new AssetBuy("wkn", startDate, amount, 0.0, 1.0, null, null));
        return asset1;
    }
